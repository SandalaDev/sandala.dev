...
> = ({ children, className, ref, hideIcon, variant = 'default', ...props }) => {
  const baseDefault =
  ...
  const baseUnstyled =
    'flex h-10 w-auto items-center justify-between rounded bg-transparent px-0 py-0 text-inherit focus:outline-none ring-0 focus:ring-0 focus:ring-offset-0 border-0 data-[state=open]:ring-0 data-[state=open]:border-0'

  return (
    <SelectPrimitive.Trigger
      className={cn(variant === 'unstyled' ? baseUnstyled : baseDefault, className)}
      ref={ref}
      {...props}
    >
      {children}
...
> = ({ children, className, position = 'popper', ref, ...props }) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      className={cn(
        'relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded bg-card text-popover-foreground shadow-md border-0 ring-0 outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
        position === 'popper' &&
